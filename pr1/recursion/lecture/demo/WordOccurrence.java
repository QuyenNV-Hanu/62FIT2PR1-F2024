...
public class WordOccurrence {
	...
	public static void main(String[] args) throws IOException {
		...
		String searchingWord = sc.nextLine();
		writeDemoDataToFile();
		searchWordInDirectory("./demo", searchingWord);
	}

	public static void searchWordInDirectory(String directory, String word) throws IOException {
		Path dirPath = Paths.get(directory);
		Files.walk(dirPath).filter(Files::isRegularFile) // filters for files only
				.forEach(filePath -> {
					try {
						...
						for (int i = 0; i < lines.size(); i++) {
							if (lines.get(i).contains(word)) {
								System.out.println("Found in " + filePath + " at line " + (i + 1));
							}
						}
					} catch (IOException e) {
